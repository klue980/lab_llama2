...
class Llama:
    ...
    def generate(
        ...
        if os.path.exists(file_path):
            ...
            df = pd.DataFrame(columns=["Batch Size", "Seq Len", "Task", "Execution Time(us)"])
        ...
        execution_times.append((params.max_batch_size, params.max_seq_len, "Summarization", execution_time_summarization))
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            execution_times.append((params.max_batch_size, params.max_seq_len, "Generation", execution_time_generation))
        
        # Export to a table using pandas with Append new data to the DataFrame
        new_data = pd.DataFrame(execution_times, columns=["Batch Size", "Seq Len", "Task", "Execution Time(us)"])
        ...
        df = pd.concat([df, avg_df_summarization, avg_df_generation], ignore_index=True)
        df.to_csv(file_path, index=False)
        # avg_df = pd.concat([avg_df_summarization, avg_df_generation], ignore_index=True)
        # avg_df.to_csv("avg_execution_times.csv", index=False)
